...
export async function POST(request) {
  try {
    await requireAdminUser(cookies())
    const { payload, file } = await extractPayload(request)
    const record = await createRecord(RESOURCE, payload, file)
...
export async function PUT(request) {
  try {
    await requireAdminUser(cookies())
    const { payload, file } = await extractPayload(request)
    const record = await updateRecord(RESOURCE, payload, file)
    return NextResponse.json({ record })
  } catch (error) {
